...
export default function BugReporting() {
    ...
    const [newBug, setNewBug] = useState<any>({
        title: '',
        status: 'Open',
        priority: 'Medium',
        reporter: ''
    ...
    const [isAddDialogOpen, setIsAddDialogOpen] = useState<any>(false)
    ...
    const addBug = () => {
        const id = bugs.length > 0 ? Math.max(...bugs.map((bug:any) => bug.id)) + 1 : 1
    ...
    const updateBug = () => {
        if (editingBug) {
            setBugs(bugs.map((bug:any) => bug.id === editingBug.id ? editingBug : bug))
            setEditingBug(null)
            setIsEditDialogOpen(false)
        }
    }

    const deleteBug = (id:any) => {
        setBugs(bugs.filter((bug: any) => bug.id !== id))
    }
